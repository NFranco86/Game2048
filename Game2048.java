...
public class Game2048 extends JPanel {
	...
	// TODO: function to check if the board is full, use the availableSpace function
	...
	// since availableSpace is referring to a list and the list size
	// decreasing from 16, I assumed that full would mean that when
	// that list is equal to 0, the list is full
	...
	boolean canMove() {
		...
		// given the in class notes you gave us regarding the board
		// and how it checked the horizontal and vertical spaces
		for (int x = 0; x < 4; x++) {
			for (int y = 0; y < 4; y++) {
				Tile t = tileAt(x, y);
	...
	// a function to combine 2 tiles in a given line and return it's result in an
	// array of tiles
	...
	// function to get the elements inside the line horizontally as index stays
	// constant
	private Tile[] getLine(int index) {
		Tile[] result = new Tile[4];
		// TODO: handle logic to get the elements in the board into result
		// code given to us on the project docx
		for (int i = 0; i < 4; i++) {
	...
	// copy the elements of re starting from 0 to the end TO myTiles starting at
	// index*4
	...
	private void drawTile(Graphics g2, Tile tile, int x, int y) {
		...
		// aliasing is abrupt changes in color as the information for the GUI is
		// loading, so
		...
		// if value less than 100 then size is 36 if value less than 1000 then 1000
		// otherwise
		...
		g.setColor(new Color(78, 139, 202));
		g.drawString("Score: " + myScore, 30, 350);
		...
		g.setFont(new Font(FONT_NAME, Font.PLAIN, 18));
		g.setColor(new Color(78, 139, 202));
		g.drawString("HighScore: " + highScore, 160, 350);
	}
	...
	public void CheckScore() {
		...
		if (myScore > Integer.parseInt((highScore))) {
			highScore = "" + myScore;

			File myScoreFile = new File("highscore.dat");
			...
			try {
				writeFile = new FileWriter(myScoreFile);
	...
	static class Tile {
		...
		public boolean isEmpty() {
			...
			// if a tile is empty then it should have no
			// instantiation assigned which would make it
			// have a value of zero so for the boolean, it
